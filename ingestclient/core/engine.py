...
class Engine(object):
    def __init__(
        ...
        """
        self.config = None
        self.msg_wait_iterations = (
            20
        )  # Each iteration waits for 10 seconds for incoming messages
        self.backend = None
        self.validator = None
    ...
    def configure_from_file(self, config_file):
        ...
        try:
            ...
            raise ConfigFileError(
                "Ingest Configuration File not found.  Double check the provided path: {}".format(
                    config_file
                )
            )
        except OSError as _:
    ...
    def configure(self, configuration):
        ...
        if (
            "ingest_type" not in self.config.config_data["ingest_job"]
            or self.config.config_data["ingest_job"]["ingest_type"] == "tile"
        ):
        ...
        if (
            "ingest_type" in self.config.config_data["ingest_job"]
            and self.config.config_data["ingest_job"]["ingest_type"] == "volumetric"
        ):
            self.chunk_processor = self.config.chunk_processor_class
            self.chunk_processor.setup(self.config.get_chunk_processor_params())
            self.num_x_cuboids = (
                self.config.config_data["ingest_job"]["chunk_size"]["x"]
                // BOSS_CUBOID_X
            )
            self.num_y_cuboids = (
                self.config.config_data["ingest_job"]["chunk_size"]["y"]
                // BOSS_CUBOID_Y
            )
            self.num_z_cuboids = (
                self.config.config_data["ingest_job"]["chunk_size"]["z"]
                // BOSS_CUBOID_Z
            )
    ...
    def join(self):
        ...
        """
        self.job_status, self.credentials, self.upload_job_queue, self.tile_index_queue, self.tile_bucket, self.job_params, self.tile_count = self.backend.join(
            self.ingest_job_id
        )
        ...
        always_log_info(
            "(pid={}) JOINED INGEST JOB: {}".format(os.getpid(), self.ingest_job_id)
        )
    ...
    def monitor(self, workers):
        ...
        while True:
            total_seconds = (
                datetime.datetime.now() - self.credential_create_time
            ).total_seconds()
            if total_seconds > self.backend.credential_timeout:
                self.logger.warning(
                    "(pid={}) Credentials are expiring soon, attempting to renew credentials".format(
                        os.getpid()
                    )
                )
                self.join()
                always_log_info(
                    "(pid={}) Credentials refreshed successfully".format(os.getpid())
                )
            ...
            if (time.time() - print_time) > 30:
                ...
                if status:
                    if status["current_message_count"] != 0:
                        ...
                        log_str += " - Approx {:d} of {:d} {} remaining, ETA {} minutes".format(
                            status["current_message_count"],
                            status["total_message_count"],
                            units,
                            int(status["current_message_count"] / avg_tile_rate),
                        )
    ...
    def run(self):
        ...
        if not self.credentials:
            msg = "(pid={}) Cannot start ingest engine.  Credentials not successfully received from the ingest service.".format(
                os.getpid()
            )
        ...
        if self.job_status == 0:
            msg = "(pid={}) Cannot start ingest engine.  Ingest job is not ready yet.".format(
                os.getpid()
            )
            self.logger.error(msg)
            raise Exception(msg)
        ...
        if self.job_status == 2:
            msg = "(pid={}) Ingest job already completed. Skipping ingest engine start.".format(
                os.getpid()
            )
        ...
        while True:
            ...
            if (
                ...
                self.logger.error(
                    "Invalid ingest_type specified: {}".format(
                        self.config.config_data["ingest_job"]["ingest_type"]
                    )
                )
                return
    ...
    def upload_tile(self, msg, message_id, receipt_handle):
        ...
        self.logger.info(
            "(pid={}) Processing Task -  X:{} Y:{} Z:{} T:{}".format(
                os.getpid(),
                key_parts["x_index"],
                key_parts["y_index"],
                key_parts["z_index"],
        ...
        # Call path processor
        filename = self.path_processor.process(
            key_parts["x_index"],
            key_parts["y_index"],
            key_parts["z_index"],
            key_parts["t_index"],
        ...
        # Call tile processor
        handle = self.tile_processor.process(
            filename,
            key_parts["x_index"],
            key_parts["y_index"],
            key_parts["z_index"],
        ...
        try:
            ...
            handle.seek(0)
            response = self.backend.bucket.put_object(
                ACL="private",
                Body=handle,
                Key=msg["tile_key"],
                Metadata={
            ...
            self.logger.info(
                "(pid={}) Successfully wrote file: {}".format(os.getpid(), response.key)
            )

        except Exception as e:
            self.logger.error(
                "(pid={}) Upload Tile Failed -  X:{} Y:{} Z:{} T:{} - {}".format(
                    os.getpid(),
                    key_parts["x_index"],
                    key_parts["y_index"],
                    key_parts["z_index"],
            ...
            if str(e).startswith(
                ...
                if self.access_denied_count >= 20:
                    self.logger.error(
                        "(pid={}) failed 20 times with same error, breaking out of loop: {} ".format(
                            os.getpid(), e
                        )
                    )
                    return False
        ...
        max_put_retries = 3
        if not self.backend.put_task(
            json.dumps(metadata, separators=(",", ":")), max_put_retries
        ):
            return False
    ...
    def upload_chunk(self, msg, message_id, receipt_handle):
        ...
        self.logger.info(
            "(pid={}) Processing Chunk -  X:{} Y:{} Z:{} ".format(
                os.getpid(),
                key_parts["x_index"],
                key_parts["y_index"],
                key_parts["z_index"],
        ...
        chunk, array_order = self.chunk_processor.process(
            filename, key_parts["x_index"], key_parts["y_index"], key_parts["z_index"]
        )
    ...
    def upload_cuboid(self, chunk, x, y, z, key, chunk_key, array_order):
        ...
        try:
            ...
            if array_order == XYZ_ORDER or array_order == XYZT_ORDER:
                raw_sub_chunk = chunk[
                    x : x + BOSS_CUBOID_X, y : y + BOSS_CUBOID_Y, z : z + BOSS_CUBOID_Z
                ]
                # Fix ordering so either Z or T first.
                ordered_array = np.transpose(raw_sub_chunk)
            else:
                ordered_array = chunk[
                    z : z + BOSS_CUBOID_Z, y : y + BOSS_CUBOID_Y, x : x + BOSS_CUBOID_X
                ]
            ...
            if array_4d.shape != (1, BOSS_CUBOID_Z, BOSS_CUBOID_Y, BOSS_CUBOID_X):
                ...
                array_4d = np.pad(
                    array_4d,
                    ((0, 0), (0, z_diff), (0, y_diff), (0, x_diff)),
                    "constant",
                    constant_values=0,
                )
            ...
            response = self.backend.volumetric_bucket.put_object(
                ACL="private",
                Body=compressed_data,
                Key=key,
                Metadata={"metadata": json.dumps(metadata, separators=(",", ":"))},
                StorageClass="STANDARD",
            )
            self.logger.info(
                "(pid={}) Successfully uploaded cuboid: {}".format(
                    os.getpid(), response.key
                )
            )

        except Exception as e:
            self.logger.error(
                "(pid={}) Upload Cuboid Failed -  X:{} Y:{} Z:{} - {}".format(
                    os.getpid(), x, y, z, e
                )
            )
            if str(e).startswith(
