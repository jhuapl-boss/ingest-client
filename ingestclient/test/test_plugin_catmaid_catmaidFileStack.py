...
class TestCatmaidFileImageStack(unittest.TestCase):
    ...
    def test_CatmaidFileImageStackPathProcessor_setup(self):
        """Test setting up the path processor"""
        pp = self.config.path_processor_class
        pp.setup(self.config.get_path_processor_params())

        assert pp.parameters["root_dir"] == resource_filename("ingestclient", "test/data/example_catmaid_stack")
    ...
    def test_CatmaidFileImageStackPathProcessor_process(self):
        ...
        pp = self.config.path_processor_class
        pp.setup(self.config.get_path_processor_params())
        assert pp.process(0, 0, 0, 0) == "{}/0/0_0_0.png".format(pp.parameters["root_dir"])
        assert pp.process(0, 1, 0, 0) == "{}/0/1_0_0.png".format(pp.parameters["root_dir"])
        assert pp.process(0, 1, 1, 0) == "{}/1/1_0_0.png".format(pp.parameters["root_dir"])

    def test_CatmaidFileImageStackPathProcessor_process_invalid(self):
    ...
    def test_CatmaidFileImageStackTileProcessor_setup(self):
        """Test setting up the tile processor"""
        tp = self.config.tile_processor_class
        tp.setup(self.config.get_tile_processor_params())

        assert tp.parameters["filetype"] == "png"
        assert tp.parameters["ingest_job"]["extent"]["y"] == [0, 1024]
    ...
    def test_CatmaidFileImageStackTileProcessor_process(self):
        ...
        # Open original data
        truth_file = os.path.join(resource_filename("ingestclient", "test/data/example_catmaid_stack/0"), "0_0_0.png")
    ...
    def test_CatmaidFileImageStackTileProcessor_process2(self):
        ...
        filename = pp.process(0, 1, 1, 0)
        handle = tp.process(filename, 0, 1, 1, 0)
        ...
        # Open original data
        truth_file = os.path.join(resource_filename("ingestclient", "test/data/example_catmaid_stack/1"), "1_0_0.png")
        truth_img = Image.open(truth_file)
        truth_img = np.array(truth_img, dtype="uint8")
    ...
    @classmethod
    def setUpClass(cls):
        cls.config_file = os.path.join(resource_filename("ingestclient", "test/data"), "boss-v0.1-catmaidStack.json")
